...
public protocol Documented {
	...
	/// The type of `self.summary`.
	associatedtype Summary

	/// The type of `self.overview`.
	associatedtype Overview

	/// The type of `self.explanation`.
	associatedtype Explanation

	/// A phrase that describes what `self` is called.
	var title: Title { get }

	/// A sentence that describes what `self` is for.
	var summary: Summary { get }

	/// A paragraph that describes what `self` has and what it does.
	var overview: Overview { get }

	/// An article that describes how `self` is made.
